...
import ru.danilarassokhin.progressive.exception.BeanConflictException;
import ru.danilarassokhin.progressive.injection.DIContainer;

/**
 * Instantiates objects from their classes
 * and uses {@link ru.danilarassokhin.progressive.injection.DIContainer}
 * for auto injection.
 */
public abstract class BasicComponentCreator {
   ...
   *
   * @param isHandlesEnabled if true will use {@link java.lang.invoke.MethodHandles} to
   *                         invoke methods or use
  ...
  /**
   * Creates object from given class and makes auto injection
   * for fields and methods if they annotated as @Autofill.
   *
  ...
  public static <C> C create(Class<C> componentClass, Object... args) {
    ...
    try {
      ...
      if (autoInjectConstructors.size() > 1) {
        throw new BeanConflictException("Found more than one constructor in "
            + componentClass.getName()
            + " annotated as @Autofill. What to use?");
      }
      if (autoInjectConstructors.size() == 1) {
        Constructor<?> constructor = autoInjectConstructors.get(0);
        args = injectBeansToParameters(
            componentClass,
      ...
      e.printStackTrace();
      throw new RuntimeException("Could not instantiate component "
          + componentClass.getName() + "! Component must have such a method: " + e.getMessage());
    }
  }

  public static List<Constructor<?>> foundAutoInjectConstructors(Constructor<?>[] constructors) {
    return Arrays.stream(constructors).parallel().unordered()
        .filter(c -> c.isAnnotationPresent(Autofill.class))
        .collect(Collectors.toList());
  }

  public static Object[] injectBeansToParameters(Class<?> beanClass,
  ...
  public static Qualifier findQualifierAnnotation(Class<?> beanClass, Annotation[] annotations) {
    ...
    List<Annotation> qualifiers = Arrays.stream(annotations).parallel().unordered()
